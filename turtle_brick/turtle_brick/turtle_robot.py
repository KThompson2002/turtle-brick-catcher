...
class TurtleRobot(Node):
    ...
    def __init__(self):
        """Initialize the turtle robot node."""
        ...
        self._goal = self.create_subscription(
    ...
    def timer_callback(self):
        ...
        if self.state == State.START and self.pose is not None:
            ...
            if (
                get_distance(
                    [self.curr_waypoint.x, self.curr_waypoint.y],
    ...
    def translate_robot(self):
        ...
        if self.pose is not None:
            ...
            odom_trans.transform.rotation = quatToMsg(
                axangle2quat([0, 0, 1.0], self.pose.theta)
            )
        odom_trans.header.stamp = self.get_clock().now().to_msg()
    ...
    def publish_joints(self):
        ...
        wheel_joint: Spin the wheel proportional to movement
        """
        joint_msg = JointState()
        joint_msg.header.stamp = self.get_clock().now().to_msg()
        joint_msg.name = ['platform_joint', 'stem_joint', 'wheel_joint']
        ...
        if self.curr_vel is not None:
            ...
            self.wheel_angle = math.atan2(
                math.sin(self.wheel_angle), math.cos(self.wheel_angle)
            )

        joint_msg.position = [
            self.platform_tilt, self.stem_angle, self.wheel_angle
        ]
        self._joint.publish(joint_msg)
